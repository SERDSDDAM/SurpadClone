...
export class AdvancedRBACService {
    ...
    try {
      ...
      for (const perm of conditionalPerms) {
        ...
        return {
          ...
          requiresApproval: perm.requiresApprovalFrom ? perm.requiresApprovalFrom.length > 0 : false,
  ...
  private async checkTemporaryPermissions(
    ...
    try {
      ...
      for (const perm of tempPerms) {
        // فحص حد الاستخدام
        const currentUsage = perm.currentUsage || 0;
        if (perm.maxUsage && currentUsage >= perm.maxUsage) {
          continue;
        }
        ...
        await db
          ...
          .set({
            ...
            lastUsedAt: now,
            usageLog: sql`COALESCE(${temporaryPermissions.usageLog}, '[]'::jsonb) || ${JSON.stringify([usageLogEntry])}::jsonb`
          })
        ...
        return {
          ...
          conditions: {
            ...
            remainingUsage: perm.maxUsage ? perm.maxUsage - currentUsage - 1 : undefined
          }
        };
      }
    } catch (error) {
      console.error('Error checking temporary permissions:', error);
    }
  ...
  private async checkContextualRoles(
    ...
    try {
      ...
      for (const role of contextualRolesList) {
        ...
        if (contextMatches) {
          ...
          if (!role.isCurrentlyActive) {
            const activationCount = role.activationCount || 0;
            await db
      ...
      console.error('Error checking contextual roles:', error);
    }
    ...
    try {
      ...
      for (const delegation of activeDelegations) {
        if (delegation.delegatedPermissions.includes(permissionCode)) {
          ...
          if (conditionsOk) {
            // تحديث سجل الاستخدام
            const usageCount = delegation.usageCount || 0;
            const usageLogEntry = {
              timestamp: now.toISOString(),
            ...
            await db
              ...
              .set({
                ...
                lastUsedAt: now,
                usageLog: sql`COALESCE(${smartDelegations.usageLog}, '[]'::jsonb) || ${JSON.stringify([usageLogEntry])}::jsonb`
              })
              .where(eq(smartDelegations.id, delegation.id));
            
            const conditions = delegation.conditions as any;
            return {
              granted: true,
      ...
      console.error('Error checking active delegations:', error);
    }
  ...
  private async generateSecurityAlert(
    ...
    const alertType = result.granted ? 'security' : 'compliance';
    ...
    try {
      const recipients = await this.getSecurityTeamRecipients();
  ...
  private async generateUsageAlert(
    ...
    try {
      ...
      console.error('Error generating usage alert:', error);
    }
  }
  ...
  private async checkContextMatch(role: ContextualRole, context: UserContext): Promise<boolean> {
    try {
      ...
      console.error('Error checking context match:', error);
      return false;
    }
  }
  ...
  private async checkDelegationConditions(delegation: SmartDelegation, context: UserContext): Promise<boolean> {
    try {
      ...
      console.error('Error checking delegation conditions:', error);
      return false;
    }
  }
  ...
  private async getRecentAccessAttempts(userId: string, action: string, resource: string): Promise<number> {
    try {
      ...
      console.error('Error getting recent access attempts:', error);
      return 0;
    }
  }
